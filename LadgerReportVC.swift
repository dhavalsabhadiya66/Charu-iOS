...
class LadgerReportVC: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, AppNavigationControllerDelegate, JDSelectionDelegate{
    ...
    fileprivate var arrAccounts = [typeAliasDictionary]()
    fileprivate var arrSeleAccount = [typeAliasDictionary]()
    ...
    override func viewDidLoad() {
        ...
        self.tfAccount.setRightImage(UIImage(named: "ic_drop_down")!.tinted(with: .darkGray))
    }
...
//        self.tfToDate.text = dateFormatter.string(from: toDate)
    ...
    func setUpNavigationBar(){
        Obj_AppDelegate.navigationController.setCustomTitle("Ledger Report")
    ...
    func appNavigationController_RightMenuAction() {
        ...
        var data = ""
        data += "<TABLE border=\"1\">"
        data += "<TR><td align='center' colspan=\"15\"><b>Ledger Report</b></td>";
        data += "</TR>";
        ...
        data += "<TH align='center' style=\"white-space: nowrap;font-size:10px;\"><b>Trans Type</b></TH>";
        data += "<TH align='center' style=\"white-space: nowrap;font-size:10px;\"><b>Cheque No</b></TH>";
        data += "<TH align='center' style=\"white-space: nowrap;font-size:10px;\"><b>Trans Mode</b></TH>";
        ...
        for dict in self.arrReport{
            ...
            let transType = dict.isKeyNull(RES_KEY.Trans_Type) ? "-":"\(dict[RES_KEY.Trans_Type]!)"
            let chequeNo = dict.isKeyNull(RES_KEY.Cheque_No) ? "-":"\(dict[RES_KEY.Cheque_No]!)"
            let transMode = dict.isKeyNull(RES_KEY.Trans_Mode) ? "-":"\(dict[RES_KEY.Trans_Mode]!)"
            ...
            data += "<TD align='center' style =\"white-space: nowrap;font-size:7px;\">\(transType)</TD>";
            data += "<TD align='right' style =\"white-space: nowrap;font-size:7px;\">\(chequeNo)</TD>";
            data += "<TD align='center' style =\"white-space: nowrap;font-size:7px;\">\(transMode)</TD>";
        ...
        let filePath = "\(directory)/ladgerReport.pdf"
    ...
    func getAccountList(){
        ...
        let param = [REQ_KEY.CompCode: "\(userInfo[RES_KEY.CompCode] ?? "" as AnyObject)",
                     REQ_KEY.AccountCode: "",
                     REQ_KEY.AccountName: ""]
        Obj_OperationApi.callWebService(ApiMethod.AccountList, method: .post, parameters: param, onCompletion: { (responce) in
            self.arrAccounts = responce[RES_KEY.Data] as? [typeAliasDictionary] ?? [typeAliasDictionary]()
        }) { (errStr) in
    ...
    func updateTotalView(){
        ...
        for dict in self.arrReport{
            ...
            totalDebitAmt += round((debitAmt*100)/100)
            totalCreditAmt += round((creditAmt*100)/100)
        }
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        if textField == self.tfAccount{
            ...
            if self.arrAccounts.count == 0 {
                DesignModel.showMessage("Account not found", for: self)
                return false
            }
            var isAbleSearch = false
            if self.arrAccounts.count > 5 { isAbleSearch = true }
            let _JDSelection = JDSelection.init(title: "Select Account", selectionType: .SINGLE, listArray: self.arrAccounts, selectedArray: self.arrSeleAccount, keyDictionary: keys, inputType: INPUT_TYPE.ACCOUNT, isSelectionCompulsory: true, contentFrame: CGRect(x: CGFloat(0), y: CGFloat(0), width: CGFloat(self.view.frame.width * 0.75), height: CGFloat(self.view.frame.height * 0.5)), isPaginationExist: isAbleSearch, isRemoveOnDoneClick: true)
    ...
    func selectedOption(arrSelected: [typeAliasDictionary], inputType: INPUT_TYPE) {
        if inputType == .ACCOUNT{
            self.arrSeleAccount = arrSelected
            if let dict = arrSelected.first{
                self.tfAccount.text = dict.isKeyNull(RES_KEY.Ac_Name) ? "-":"\(dict[RES_KEY.Ac_Name]!)"
            }else {
                self.tfAccount.text = ""
            }
        }
    ...
    @IBAction func reportBtnAction(_ sender: UIButton){
        ...
        if self.arrSeleAccount.count < 1 {
        ...
        dateFormatter.dateFormat = "dd-MMM-yyyy"
        let acDict = self.arrSeleAccount.first!
        let param = [REQ_KEY.CompCode: "\(userInfo[RES_KEY.CompCode] ?? "" as AnyObject)",
    ...
    @IBAction func clearBtnActio(_ sender: UIButton){
        ...
        self.tfAccount.text = ""
        self.arrSeleAccount = [typeAliasDictionary]()
        self.arrReport = [typeAliasDictionary]()
        self.updateTotalView()
        self.tableReport.reloadData()
        self.viewReportBG.isHidden = true
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "LedgerReportCell", for: indexPath) as? LedgerReportCell{
            ...
            cell.lblTransType.text = dict.isKeyNull(RES_KEY.Trans_Type) ? "-":"\(dict[RES_KEY.Trans_Type]!)"
            cell.lblChequeNo.text = dict.isKeyNull(RES_KEY.Cheque_No) ? "-":"\(dict[RES_KEY.Cheque_No]!)"
            cell.lblTransMode.text = dict.isKeyNull(RES_KEY.Trans_Mode) ? "-":"\(dict[RES_KEY.Trans_Mode]!)"
...
func withSeprate(val: Any) -> String{
    let formatter = NumberFormatter()
    formatter.groupingSeparator = ","
    formatter.groupingSize = 3
    formatter.secondaryGroupingSize = 2
    ...
    formatter.numberStyle = .decimal
    return String(formatter.string(for: val)!)
}
